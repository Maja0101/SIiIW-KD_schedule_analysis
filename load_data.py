...
def create_graph(
    ...
    for row in stop_times.itertuples(index=False):
        if row.stop_sequence == 0:
            previous_stop_id = stops.loc[row.stop_id]
            previous_stop_id = int(previous_stop_id['parent_station']) if pd.notna(previous_stop_id['parent_station']) else previous_stop_id.name
            ...
            graph.add_edge(previous_stop_id, next_stop_id, edge)
